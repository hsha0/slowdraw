...
for file_name in listdir("npy"):
    ...
    if first:
        train_images = data_one_class
    else:
        train_images = np.concatenate((train_images, data_one_class))
    ...
    if first:
        train_labels = label_one_class
        first = False
    else:
        train_labels = np.concatenate((train_labels, label_one_class))
...
drawing_classifier = tf.estimator.Estimator(
    model_fn=cnn_model_fn, model_dir="/tmp/model_test")
